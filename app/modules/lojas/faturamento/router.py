...
from app.modules.lojas.faturamento.service import resumo_faturamento
...
@router.get("/resumo")
def get_resumo_faturamento(data: str = Query(..., example="2026-01-27")):
    return resumo_faturamento(data)


@router.get("/filiais")
def get_faturamento_filiais(data: str = Query(..., example="2026-01-27")):
    return faturamento_filiais(data)


@router.get("/vendedores")
def get_faturamento_vendedores(data: str = Query(..., example="2026-01-27")):
    return faturamento_vendedores(data)


@router.get("/produtos")
def get_faturamento_produtos(data: str = Query(..., example="2026-01-27")):
    return faturamento_produtos(data)


@router.get("/periodo/resumo")
def get_resumo_faturamento_periodo(
    inicio: str = Query(..., example="2026-01-01"),
    fim: str = Query(..., example="2026-01-31"),
):
